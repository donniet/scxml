...
public final class SCXMLHelper {
    ...
    public static void runInitializationScripts(final Datamodel datamodel,
            final Context ctx, final Evaluator evaluator,
            final Log log) {
    	
    	Log myLog = log;
    	if(myLog == null) {
    		myLog = LogFactory.getLog(SCXMLHelper.class);
    	}

        
        List<Action> actions = datamodel.getActions();
        if(actions == null)
        	return;
    	...
    	for(Iterator<Action> iter = actions.iterator(); iter.hasNext(); ) {
    		...
    		if(a instanceof Script) {
    			Script s = (Script)a;
    			try {
					s.executeForInitialization(ctx, evaluator);
				} catch (SCXMLExpressionException see) {
            		myLog.error(see.getMessage(), see);
				}
    		}
    		else {
    			myLog.error("Non-<script> actions are not supported inside the <datamodel>");
    		}
    	}
    ...
    public static void cloneDatamodel(final Datamodel datamodel,
        ...
        for (Iterator iter = data.iterator(); iter.hasNext();) {
            ...
            else if (!SCXMLHelper.isStringEmpty(datum.getTextContent())) {
            	Object value = null;
            	try {
	            	ctx.setLocal(NAMESPACES_KEY, datum.getNamespaces());
	            	value = evaluator.evalData(ctx, datum.getTextContent());
            ...
            else if (!SCXMLHelper.isStringEmpty(datum.getExpr())) {
                Object value = null;
                try {
                    ctx.setLocal(NAMESPACES_KEY, datum.getNamespaces());
